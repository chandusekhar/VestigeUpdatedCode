...
namespace TransfersComponent.UI
{
    public partial class frmTOI : CoreComponent.Core.UI.Transaction
    {
            ...
            if (!invalidDestLoc)
            {
                ...
                if (query.ToList().Count > 0)
                {
                    ...
                    if (dv.Count > 0)
                    {
                        ...
                        txtLineWeight.Text = (Math.Round(Convert.ToDecimal( dv[0]["Weight"]),2)).ToString();
        ...
        void SelectSearchGrid(string tNumber, int sourceLocationId)
        {
            ...
            txtPONumber.Text = (m_objTOI.TotalTOIWeight / 1000).ToString();
        ...
        bool CheckValidTOIQuantityEntered()
        {
            ...
            if (m_lstTOIDetail != null && m_lstTOIDetail.Count > 0)
            {
                for (int i = 0; i < m_lstTOIDetail.Count; i++)
                {
                    ...
                    decimal linewieght = Convert.ToDecimal(m_lstTOIDetail[i].LineWeight);
                    ...
                    else
                    {
                        ...
                        totalTOIWeight = totalTOIWeight+ m_lstTOIDetail[i].LineWeight;
                        m_objTOI.TotalTOIWeight =  Math.Round(totalTOIWeight, Common.DisplayAmountRounding, MidpointRounding.AwayFromZero);
                        //totalTOIWeight = m_objTOI.TotalTOIWeight;
            ...
            if (txtUnitQty.Text.Length > 0 && txtAvailableQty.Text.Length > 0)
            {
                ...
                else
                {
                    ...
                    m_objTOI.TotalTOIWeight = totalTOIWeight + Convert.ToDecimal(txtLineWeight.Text) * m_toiDetail.UnitQty;
                    txtPONumber.Text = (m_objTOI.TotalTOIWeight/1000).ToString();
        ...
        bool AddItem()
        {
            ...
            m_toiDetail.LineWeight = Math.Round(Convert.ToDecimal((Convert.ToDecimal(txtLineWeight.Text) * Convert.ToDecimal(m_toiDetail.DBUnitQty))), 2);
        ...
        void ValidatedItemCode(Boolean yesNo)
        {
            ...
            if (isTextBoxEmpty == true)
            {
                ...
                txtAvailableQty.Text = string.Empty;
                txtUnitQty.Text = string.Empty;
                txtLineWeight.Text = string.Empty;
        ...
        private void cmbPriceMode_SelectedValueChanged(object sender, EventArgs e)
        {
            if (cmbPriceMode.Text == "MRP")
            {
                cmbAppDpp.SelectedValue = 2;
            }
            else if (cmbPriceMode.Text == "Item Cost") {
                cmbAppDpp.SelectedValue = 1;
            }
        }
